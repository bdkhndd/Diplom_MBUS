// admin/src/App.tsx - FULL WIDTH FIXED
...
import { 
  ...
  ArrowRight,
  type LucideProps // 'type' гэж зааж өгснөөр алдаа арилна
} from 'lucide-react';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        // 1. Хаягийг counts болгож засав
        const res = await axios.get('http://localhost/api/stats/counts');
        
        // 2. Хэрэв Controller success:true буцааж байвал датаг онооно
        if (res.data && res.data.success) {
          setCounts(res.data.data); 
        } else {
          // Хэрэв Controller шууд объект буцааж байвал:
          setCounts(res.data);
        }
      } catch (error) {
        console.error("Backend-ээс дата авахад алдаа гарлаа:", error);
        // Алдаа гарсан үед л "Filler" тоо харуулна
        setCounts({
